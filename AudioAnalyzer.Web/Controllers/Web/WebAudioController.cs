...
public class WebAudioController : Controller
{
    ...
    public WebAudioController(RabbitMqPostManager postManager, 
    ...
    [HttpGet]
    [Route("BasicInfo")]
    public IActionResult BasicInfo(int fileId)
    {
        var file = _databaseDbContextService.UploadedFileRepository.GetEntity(fileId, false);
        if (file == null)
            return NotFound();
        
        return PartialView("BasicInfo", file);
    }

    [HttpGet]
    [Route("Summary")]
    public async Task<IActionResult> Summary(int requestId, int fileId)
    {
        ...
        if (fileRequestedEvent == null)
            return BadRequest("Запрос не найден");
        ...
        return PartialView("Summary", summaryViewModel);
    }
    ...
    [HttpGet]
    [Route("Classify")]
    public async Task<IActionResult> Classify(int requestId, int fileId)
    {
        var fileRequestedEvent = await _databaseDbContextService.GetFileRequestedEventByIndex(requestId, 
            fileId,
